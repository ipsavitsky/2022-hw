...
solution* change_processor::eval(solution* sol_to_change) {
    ...
    auto get_task = std::bind(std::uniform_int_distribution<>(0, sol_to_change->get_task_amount() - 1),
                              std::default_random_engine());
    auto get_proc = std::bind(std::uniform_int_distribution<>(0, sol_to_change->get_proc_amount() - 1),
                              std::default_random_engine());
